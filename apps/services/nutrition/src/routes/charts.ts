...
import type { ChartMetric, ChartRange, ChartData, ChartDataPoint, ChartDataSummary } from '@repo/nutrition-types';
...
charts.get('/:metric', async (c) => {
  ...
  if (!validMetrics.includes(metric)) {
    throw NutritionError.badRequest(`Invalid metric. Valid: ${validMetrics.join(', ')}`);
  }
  
  // Validate range
  const validRanges = ['1d', '7d', '30d', '90d'];
  if (!validRanges.includes(range)) {
    throw NutritionError.badRequest(`Invalid range. Valid: ${validRanges.join(', ')}`);
  }
  ...
  const dataPoints = await getChartData(c.env.DB, userId, metric, startDateStr, endDateStr);
  
  // Get target if specified
  const targetParam = c.req.query('target');
  let target: number | null = targetParam ? parseFloat(targetParam) : null;
...
charts.get('/', async (c) => {
  ...
  const metrics = metricsParam ? metricsParam.split(',') as ChartMetric[] : ['calories'];
  const validMetrics = ['calories', 'protein', 'carbs', 'fat', 'fiber', 'sugar', 'sodium'];
 ...
 * Calculate summary statistics
 */
function calculateSummary(values: number[], range: ChartRange): ChartDataSummary {
  if (values.length === 0) {
    return {
      average: 0,
      minimum: 0,
