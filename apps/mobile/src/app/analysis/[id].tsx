...
export default function AnalysisReviewScreen() {
    ...
    const pollInterval = setInterval(async () => {
      try {
        ...
        if (status.status === 'completed' || status.status === 'needs_review') {
          ...
          clearInterval(pollInterval);
          setLoading(false);
          Alert.alert('Analysis Failed', status.errorMessage || 'Please try again.');
        }
      } catch {
  ...
  const confirmAnalysis = useCallback(async () => {
    ...
    try {
      ...
      Alert.alert(
        'Meal Added!',
        `Added ${result.meal.name} to your meals.`,
        [
          {
            text: 'View Meal',
  ...
  const totalFat = analysis.foods.reduce((sum, food) => sum + food.nutrition.fatG, 0);
