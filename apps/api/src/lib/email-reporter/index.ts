...
export interface EmailReportConfig {
  ...
  fromName: string;
  replyToEmail?: string;
  reportSubject: (name: string, month: string) => string;
  baseUrl: string; // For generating export links
}

/**
 * Default email configuration
 */
export const defaultEmailConfig: EmailReportConfig = {
 ...
 */
export function generateEmailHTML(data: MonthlyReportData, config: EmailReportConfig): string {
...
export async function calculateMonthlyStats(
  ...
  year: number,
  month: number
): Promise<MonthlyReportData['stats']> {
...
export async function sendMonthlyReport(
  ...
  year: number,
  month: number,
  config: EmailReportConfig = defaultEmailConfig
